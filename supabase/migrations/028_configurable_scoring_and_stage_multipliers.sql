-- ============================================================================
-- 028 — Admin-configurable scoring rules + per-stage multiplier presets
-- ----------------------------------------------------------------------------
-- 1. scoring_rules   — singleton row holding the points awarded per aspect
--                      (correct winner / home goal / away goal / exact score).
--                      Admin-editable from the dashboard; score_match() now
--                      reads from this table instead of hardcoded constants.
-- 2. stage_multipliers — one row per match_stage holding the *default*
--                      multiplier (1x-6x) the admin wants applied to matches
--                      in that stage. Setting it bulk-applies to all matches
--                      of that stage (admin can still override individual
--                      matches afterwards via the existing per-match control).
-- 3. matches.points_multiplier — allowed range extended from 1-3/1-5 to 1-6.
-- ============================================================================
begin;

-- ----------------------------------------------------------------------------
-- 1. scoring_rules (singleton, id is always 1)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2. stage_multipliers (one row per match_stage)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3. Widen matches.points_multiplier to 1-6 (drop the older, stricter checks)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4. RPC — admin sets a stage's default multiplier and bulk-applies it to
--    every match currently in that stage.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5. score_match — now reads point values from scoring_rules instead of the
--    hardcoded 5/2/2/5 constants (defaults match the admin's intended 3/1/1/5).
-- ----------------------------------------------------------------------------
...
begin
  ...
  if not found then
    -- Fall back to the documented defaults if the singleton row is missing.
    r := row(3, 1, 1, 5);
  end if;
