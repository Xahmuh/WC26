...
-- ============================================================================
-- Adds:
--   • users.email / last_login / role ('user' | 'admin')
--   • matches.points_multiplier (joker: 1x..5x)
--   • prediction_questions + user_question_predictions (tournament-wide picks)
--   • points.question_id (+ nullable match_id) so question points share the
--     leaderboard sum
--   • competition_groups + group_members (mini-leagues) with creator auto-enrol
--   • is_admin() helper + admin RLS; resolve_prediction_question() RPC
-- ============================================================================
...
begin
  ...
  update public.prediction_questions
    set correct_answer = p_correct_answer, status = 'resolved', resolved_at = now()
  ...
  where uqp.question_id = p_question_id and uqp.prediction = p_correct_answer
...
create policy "Users read own question predictions"
  on public.user_question_predictions for select to authenticated using (user_id = auth.uid());
...
create policy "Users upsert own question predictions"
  on public.user_question_predictions for insert to authenticated with check (user_id = auth.uid());
...
create policy "Users create groups"
  on public.competition_groups for insert to authenticated with check (created_by = auth.uid());
drop policy if exists "Creators delete groups" on public.competition_groups;
create policy "Creators delete groups"
  on public.competition_groups for delete to authenticated using (created_by = auth.uid());
