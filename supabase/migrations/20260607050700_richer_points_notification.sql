...
begin
  ...
  select
    ...
    coalesce(ht.name, 'Home') || ' ' || m.home_score || ' - ' || m.away_score || ' ' || coalesce(at.name, 'Away')
      ...
      || chr(10)
      || 'Winner ' || case when pt.winner_points > 0 then chr(10003) else chr(10007) end
      || ' · Home ' || case when pt.home_goal_points > 0 then chr(10003) else chr(10007) end
      || ' · Away ' || case when pt.away_goal_points > 0 then chr(10003) else chr(10007) end
      || case when pt.exact_bonus > 0 then ' · Exact ' || chr(10003) else '' end,
    jsonb_build_object(
      'match_id',         m.id,
