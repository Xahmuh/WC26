-- ============================================================================
-- 029 — Simplify scoring: drop "home goal" / "away goal" as scorable aspects.
-- New rule set: a prediction only earns points for (a) picking the correct
-- winner/draw, and (b) nailing the exact score. Partial credit for matching
-- just the home or away score is removed entirely.
--
-- e.g. predicted 2-2, actual 2-1 -> 0 points (wrong winner, not exact)
--      predicted 2-2, actual 2-2 -> winner + exact bonus
-- ============================================================================
begin;

-- 1. Drop the now-unused columns from the admin-editable scoring_rules config.
...
-- 2. score_match — stop awarding home/away-goal points; keep the `points`
--    table's home_goal_points/away_goal_points columns (for historical/
--    breakdown compatibility) but always persist them as 0.
